...
class Database():
    ...
    def get_entry(self, table_name, id):
        ...
        if target_table:
            target_entry = self.session.query(
                target_table).filter_by(id=id).first()
    ...
    def update_entry(self, table_name, values):
        ...
        entry_to_update = self.session.query(
            table).filter_by(id=values['id']).first()
    ...
    def delete_by_id(self, table_name, del_id):
        ...
        if table_to_find:
            to_delete = self.session.query(
                table_to_find).filter_by(id=del_id).first()
            if to_delete:
